...
/// \brief FemtoWorldCollisionSelection - event selection within the
/// o2femtoworld framework \author Andi Mathis, TU München,
/// andreas.mathis@ph.tum.de \author Zuzanna Chochulska, WUT Warsaw,
/// zchochul@cern.ch
...
public:
  ...
  void init(HistogramRegistry *registry) {
    ...
    mHistogramRegistry->add("Event/zvtxhist", "; vtx_{z} (cm); Entries", kTH1F,
                            {{300, -12.5, 12.5}});
    mHistogramRegistry->add("Event/MultV0M", "; vMultV0M; Entries", kTH1F,
                            {{16384, 0, 32768}});
    mHistogramRegistry->add("Event/MultT0M", "; vMultT0M; Entries", kTH1F,
                            {{4096, 0, 8192}});
    mHistogramRegistry->add("Event/MultNTracksPV", "; vMultNTracksPV; Entries",
                            kTH1F, {{120, 0, 120}});
    mHistogramRegistry->add("Event/MultNTracklets",
                            "; vMultNTrackslets; Entries", kTH1F,
                            {{300, 0, 300}});
    mHistogramRegistry->add("Event/MultTPC", "; vMultTPC; Entries", kTH1I,
                            {{600, 0, 600}});
  }
  ...
  template <typename T> void fillQA(T const &col) {
    if (mHistogramRegistry) {
      ...
      mHistogramRegistry->fill(HIST("Event/MultNTracksPV"),
                               col.multNTracksPV());
      mHistogramRegistry->fill(HIST("Event/MultNTracklets"),
                               col.multTracklets());
